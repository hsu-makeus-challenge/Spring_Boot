...
public class StoresExistValidator implements ConstraintValidator<ExistStore, Long> {
  ...
  public boolean isValid(Long value, ConstraintValidatorContext context) {
    if (value == null) {
      ...
      context
          .buildConstraintViolationWithTemplate(StoreErrorStatus.STORE_BAD_REQUEST.toString())
    ...
    if (!isValid) {
      context.disableDefaultConstraintViolation();
      context
          .buildConstraintViolationWithTemplate(StoreErrorStatus.STORE_NOT_FOUND.toString())
          .addConstraintViolation();
    }
